...
const evergreen: { slug: string; title: string; description: string; category: string; keywords: string[]; sections: ArticleSection[] }[] = [
  ...
  {
    slug: "claude-agents-memory-that-write-production-code",
    title: "Claude Agents & Memory That Write Production Code, Not Just Demo Code",
    description: "How to configure Claude Code's memory, subagents, and project instructions so the code it generates is production-grade by default — security, tests, and operations baked into the prompt instead of bolted on later.",
    category: "Production Readiness",
    keywords: ["claude code production", "claude agents best practices", "claude memory setup", "ai coding production ready"],
    sections: [
      { heading: "Vibe code all you want — the setup does the hardening", paragraphs: ["We're not here to talk anyone out of vibe coding with Claude Code. The speed is real and the leverage is enormous. The problem was never the speed; it's that a fresh agent optimizes for 'make the demo work,' and production concerns — authorization, input validation, observability — never make it into the output unless they're in the instructions. The fix isn't reviewing every line the agent writes. It's changing what 'done' means for the agent before it writes a single one.", "Get the memory and guardrails right once, and the same velocity that used to produce demo-grade code starts producing something much closer to production-grade — for free, on every future session."] },
      { heading: "Put production standards in CLAUDE.md, not in your head", paragraphs: ["A project's CLAUDE.md (or equivalent memory file) is read at the start of every session — it's the highest-leverage place to encode standards. The teams that get production code out of Claude write down the non-negotiables there: parameterized queries only, every endpoint validates input and checks authorization, no secrets in source, a test alongside each feature. Rules the agent re-reads every session beat rules you remember to enforce in review — because at AI speed, you won't.", "Keep it specific and enforceable. 'Write secure code' does nothing; 'every API route must call the auth middleware and reject unauthenticated requests with 401' changes the output."] },
      { heading: "Use subagents to review what the builder just built", paragraphs: ["A single agent writing and blessing its own code is the demo trap. A more production-shaped setup splits the work: a builder agent generates the feature, then a dedicated reviewer agent — a security auditor, a test writer — inspects the diff with fresh eyes and a different mandate. It catches the missing authorization check and the untested error path the builder was never asked to think about.", "This mirrors how a real engineering team works, and it's the cheapest quality gate you can add to a vibe-coding workflow: no extra humans, just a second agent with an adversarial job description."] },
      { heading: "Memory is a liability if it drifts", paragraphs: ["Memory that names a file, flag, or convention that no longer exists will confidently send the agent down the wrong path. Treat memory like code: keep it small, keep it true, delete what's stale, and never let it record a fix that was later reverted. A tidy, accurate memory makes every session smarter; a bloated, wrong one makes every session subtly worse.", "The same discipline applies to what you promote from a session — save the durable decision ('we chose Postgres RLS for tenant isolation'), not the transient debugging noise."] },
      { heading: "Where the setup ends and we begin", paragraphs: ["Good memory and guardrails raise the floor dramatically, but they don't replace the production layer: real infrastructure you own, a CI/CD pipeline, monitoring, tested backups, and an independent security audit. That's the work no prompt configuration produces. Our free production assessment scores your Claude-built app against 40+ checkpoints and hands you a prioritized plan — a natural next step once your agents are shipping clean code fast."] },
    ],
  },
  {
    slug: "ai-skills-guardrails-to-prevent-security-holes",
    title: "Skills & Guardrails: Keeping AI Coding Tools From Opening Security Holes",
    description: "The guardrail patterns — reusable skills, forbidden-action rules, security subagents, and CI gates — that stop AI coding assistants from generating the vulnerabilities they generate by default.",
    category: "Security",
    keywords: ["ai coding guardrails", "claude skills security", "prevent ai security vulnerabilities", "secure ai code generation"],
    sections: [
      { heading: "The default output is insecure — by statistics, not malice", paragraphs: ["AI coding tools reproduce the most statistically likely code for a prompt, and the most likely code is often the insecure one: the endpoint without an auth check, the query built by string concatenation, the API key pasted inline. It isn't sabotage; it's the training distribution. Which means the fix isn't 'a better model' — it's a set of guardrails that make the secure path the default path.", "The good news: the same vibe-coding workflow that produces these holes can be configured to close most of them automatically, before a human ever reviews the diff."] },
      { heading: "Encode secure patterns as reusable skills", paragraphs: ["A skill is a named, reusable instruction the agent applies consistently — the perfect place to pin down security patterns so you don't re-specify them every session. A 'database access' skill that mandates parameterized queries and an ORM; an 'endpoint' skill that requires input validation and an authorization check; a 'secrets' skill that forbids inline credentials and routes everything through environment variables. Once, in the skill, instead of hoping each new prompt remembers.", "Skills turn 'please be secure' from a wish into a repeatable procedure the agent follows the same way every time."] },
      { heading: "Draw hard lines with forbidden-action rules", paragraphs: ["Some things should never happen regardless of what a prompt asks: committing secrets, disabling authentication to 'make it work,' running destructive commands, weakening a previously-secure file. Encode these as explicit forbidden actions in your project instructions — hard limits the agent won't cross even when a later prompt would casually invite it. This matters most in long agentic sessions, where a multi-file edit can quietly undo a security control three files away from where you were looking.", "Guardrails you can state as 'never do X' are the ones most worth writing down, because they fail loudly instead of silently."] },
      { heading: "Add a security reviewer the builder can't skip", paragraphs: ["Pair every builder with a security-auditor subagent whose only job is to read the diff for the OWASP-style basics: missing authorization, injection, unvalidated input, exposed secrets, permissive database rules. It has no incentive to ship — only to find problems — so it catches what a build-focused agent glosses over. Run it before the code is considered done, not after it's in production.", "This is the AI-speed version of a second pair of eyes, and it scales with your commit rate instead of your headcount."] },
      { heading: "Back it all with a CI gate", paragraphs: ["Prompt-level guardrails raise the floor; a pipeline makes the floor enforceable. Secret scanning, dependency audits, and static analysis running on every push mean a guardrail that occasionally slips can't reach production — the gate catches it mechanically, regardless of which agent or human wrote the code. Prompts persuade; pipelines enforce, and you want both.", "We install exactly this layered setup — secure skills, forbidden-action rules, a security subagent, and CI gates — as part of our hardening engagements. If you'd like to know where your AI-built app stands today, our production assessment includes a security pass and is free."] },
    ],
  },
  {
    slug: "from-claude-code-prototype-to-production-gaps-to-close",
    title: "From Claude Code Prototype to Production: The Gaps You Have to Close",
    description: "A practical map of what separates a Claude Code prototype from a production system — the security, architecture, infrastructure, and operations gaps that never show up in a working demo.",
    category: "Production Readiness",
    keywords: ["claude code prototype to production", "productionize claude app", "claude code production checklist", "ai prototype to production"],
    sections: [
      { heading: "The prototype earned its keep — now the other half begins", paragraphs: ["A Claude Code prototype that works is a genuine achievement: the hard, creative part of turning an idea into running software, done in a fraction of the old time. Keep building that way. But 'runs on my machine and demos beautifully' and 'safe to hand real customers and real data' are two different bars, separated by a body of work the demo never required — and never revealed. Closing that gap is the whole job of productionization.", "None of it means the vibe-coding approach was wrong. It means there's a second, less glamorous 40% that no coding tool produces for you."] },
      { heading: "Gap 1: Security that was never in the prompt", paragraphs: ["Scan the full git history for leaked secrets and rotate anything you find; move all configuration to a secret manager. Then verify the things demos hide: every endpoint checks authorization (not just authentication), all input is validated, database access rules actually restrict per-user data, and no dependency ships a known CVE. This is the highest-priority gap because attackers scan for exactly these patterns, and AI tools produce them at scale."] },
      { heading: "Gap 2: Architecture and data that only worked at demo scale", paragraphs: ["Ten test rows hide what a hundred thousand real ones expose: N+1 queries, missing indexes, unbounded result sets, state living in surprising places. Profile your slowest paths under realistic data volume before launch, not after the first slow-app complaint. A prototype makes locally-reasonable choices each session; production needs someone to check that they add up to a system that holds at scale."] },
      { heading: "Gap 3: Prototype infrastructure carrying real load", paragraphs: ["Preview URLs, workspace-tied databases, and free tiers are for prototypes, not paying customers. Production needs infrastructure you own and can reproduce: separate staging and production environments, a managed database with automated (and test-restored) backups, TLS everywhere, and a CI/CD pipeline so deploys are boring and rollbacks are one click. Define it as infrastructure-as-code so the environment is reviewable, not a hand-configured mystery."] },
      { heading: "Gap 4: No eyes and no recovery story", paragraphs: ["Claude Code generates approximately zero observability, so assume it's absent until you add it: error tracking, uptime checks on the journeys that matter, structured logs, and alerts routed somewhere a human reads. Then answer the two questions that define a production system — 'how would we know about an outage before a customer tells us?' and 'if the database vanished right now, how much do we lose and how fast are we back?' Blank answers there are your real risk report.", "Closing all four gaps on a typical Claude-built app is a bounded two-to-six-week engagement. If you'd like a scored map of where yours stands and what to fix first, our production readiness assessment is free and takes about 48 hours."] },
    ],
  },
];
